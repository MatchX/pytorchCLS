...
class kitchen_chefuniformDataSet(torch.utils.data.dataset.Dataset):
    """
    数据加载器
    分类数据标签序号必须从0开始并且连续
...
class dataloaderMaker(object):
    ...
    def makeroad_dataset(modelconfig, rank=-1):
        ...
        with torch_distributed_zero_first(rank):
            datasets = kitchen_chefuniformDataSet(root_path=dataconfig.traindata, dataconfig=dataconfig, istrain=True)
        ...
        if rank in [-1, 0]:
            if len(dataconfig.valdata) > 0:
                with torch_distributed_zero_first(rank):
                    testset = kitchen_chefuniformDataSet(dataconfig.valdata, dataconfig, istrain=False)
