...
class modelconfig(configbase.configBase):
    ...
    def _makemodconfig(self):
        ...
        生成模型配置文件
        """
        self.modcfg.train_type = "kitchen_chefuniform"
    ...
    def _makedataconfig(self):
        ...
        if platformname == 'windows':
            predir4train = r"F:\0match\AIdata\kitchenUniform\uniform_class/"
        else:  # linux
            predir4train = "/home/huangjunjie/datasets/chufang/uniform_class/"
        self.datacfg.traindata = [
            predir4train + "train_20250303/",
        ]
        ...
        self.datacfg.valdata = [
            predir4test + "train_20250303/",
        ]

        # 标签类型与对应目录配置
        self.datacfg.label_map = {"nochefuniform": 0, "chefuniform": 1, "uncertain": 2}
        # 是否重新生成标签文本
        ...
        new_extrate = 0.0
        torchtransseq = transforms.Compose([
            configbase.RandomCrop(pos_random_rate, cap_ext_rate, new_expansion_rate=new_extrate, to_bgr=False),  # set to_bgr=False
        ...
        torchtransseq = transforms.Compose([
            configbase.CenterCrop((new_extrate + 1)/(cap_ext_rate + 1), to_bgr=False),  # set to_bgr=False
            transforms.Resize(self.datacfg.traininputsize),  # [H,W] format
            transforms.ToTensor(),
            transforms.Normalize(mean=self.datacfg.data_mean, std=self.datacfg.std_mean),
        ])
    ...
    def _makehypconfig(self):
        ...
        生成超参数配置文件
        """
        self.hypcfg.log_root = "workdir_chefuniform/"  # log root dir
...
if __name__ == '__main__':
    import importlib
    cfgfile = "kitchen_chefuniform_configv1"
    modelconfig = None
